function func(id){
	...
	$.post("https://durak54.com.tr/tr/admin/driver_list", {id:id}, function(result){
...
function funcU(id){
	$.post("https://durak54.com.tr/tr/admin/customer_list", {id:id}, function(result){                       	document.getElementById('edit'+id).innerHTML =result;     
                        });
...
function edit(id){
	...
	var plate = document.getElementById("plate|"+id).value;
	var marka = document.getElementById("marka|"+id).value;
	var model = document.getElementById("model|"+id).value;
	
	$.post("https://durak54.com.tr/tr/admin/driver_list", {save_edit:id,uname:uname,email:email,telno:telno,plate:plate,marka:marka,model:model}, function(result){
...
function editU(id){
	...
	$.post("https://durak54.com.tr/tr/admin/customer_list", {save_edit:id,uname:uname,email:email,telno:telno}, function(result){
...
$(document).ready(function(){
    ...
    // }
    //
    // if($('.loading-logo.drivers').length>0){
    //     setTimeout(function(){
    ...
    $(document).on('click','.drivers-information .approved-icon.call', function(){
        $(this).addClass('in-process');
        $(this).find('button').focusout();
    });
    ...
    $(document).on('click','.drivers-information .approved-icon.call.in-process', function(){
        $(this).removeClass('in-process');
        $(this).find('button').focusout();
    });
...
$( "#close_info" ).click(function() {
    document.querySelector('#calltaksi').classList.add('hidden');
    //kapatma tuşu
});
...
function getMapCoords(){
    ...
    if ( navigator.geolocation ){
        ...
        navigator.geolocation.getCurrentPosition(function(pos){
            ...
            var harita = new google.maps.Map(document.getElementById("map"), haritaAyarlari);


            /**
             * Bulunduğumuz Yeri İşaretleyelim
             */
            var user_icon = "https://durak54.com.tr/tr/public/customer/images/kiz.png";
            var marker = new google.maps.Marker({
...
function request_location()
{
    ...
    setTimeout(request_location, 2*2000);
...
var markers = []; // Create a marker array to hold your markers
 var json;
var send_id;
	var kontrol=0;					var basilantaksi;
var veri23= new Array();
...
{
	let markers = [];
	 var user_icone = "https://durak54.com.tr/tr/public/customer/images/erkek.png";
var taxi_icon = "https://durak54.com.tr/tr/public/customer/images/parking_lot_maps.png";
    // For simplicity of example we'll
    // send POST AJAX request with jQuery
    $.post("https://durak54.com.tr/tr/index",
        {
        ...
        function(){
            ...
            var pos = {
   			...
   			if (temp == 0){
                ...
                var harita = new google.maps.Map(document.getElementById("map"), {
                    ...
                    zoom: 18
 					...
 					$.post("https://durak54.com.tr/tr/check_driver", {res:1}, function(result){
                            
                            veri23=JSON.parse(result);

						
	function setMarkers() {

    for (var i = 0; i < veri23.length; i++) {
       
        var myLatLng = new google.maps.LatLng(veri23[i].latitude, veri23[i].longitude);
        var marker = new google.maps.Marker({
            position: myLatLng,
           map: harita,
            animation: google.maps.Animation.BOUNCE,
            title: veri23[i].uname,
            zIndex: veri23[i].id,
			store_id: veri23[i].id,
			icon:taxi_icon
        });
	...
	google.maps.event.addListener(marker,'click', (function(marker,id){
                    ...
                    var tloc = veri23[i].latitude;
                    var tloc2 = veri23[i].longitude;
				


                    return function() {
						
                        $.post("https://durak54.com.tr/tr/index", {id: marker.get('store_id'), tloc: tloc, tloc2: tloc2, uloc:position.coords.latitude, uloc2:position.coords.longitude}, function(result){
                             basilantaksi=marker.get('store_id');
                            document.querySelector('#calltaksi').classList.remove('hidden');
                            document.querySelector('#taxi_info').innerHTML=result;
                        });
	  ...
	  var myYLatLng = new google.maps.LatLng(position.coords.latitude, position.coords.longitude);
        var marker = new google.maps.Marker({
            position: myYLatLng,
           map: harita,
          //  animation: google.maps.Animation.BOUNCE,
...
function reloadMarkers() {
 ...
 //  reloadMarkers();
					//setMarkers();
   //document.getElementById('reloadMarkers').addEventListener('click', setMarkers);
 //document.getElementById('reloadMarkers2').addEventListener('click', reloadMarkers);
						setInterval(function(){ reloadMarkers(); }, 2000);
});
...
$( "#taxi_button" ).click(function() {
  navigator.geolocation.getCurrentPosition(function(pos){
	 
        $.post("https://durak54.com.tr/tr/create_call", {
		
            id:basilantaksi,
            uloc: pos.coords.latitude,
            uloc2: pos.coords.longitude

        }, function (result) {
            document.querySelector('#taxi_info').innerHTML=result;
            document.querySelector('#calltaksi').classList.remove('hidden');
            document.querySelector('#calldiv').innerHTML='<button id="basara" value="'+basilantaksi+'" onclick="myFunction()" name="create_call" type="button" class="btn btn-primary">Taksiyi Ara</a>\n';
            //
            // document.querySelector('#calltaksi').classList.remove('hidden');
            // document.querySelector('#taxi_info').innerHTML = result;
        });
            ...
            navigator.geolocation.getCurrentPosition(
                ...
                () => {
                    handleLocationError(true, harita.getCenter());
                }
            );
        });
